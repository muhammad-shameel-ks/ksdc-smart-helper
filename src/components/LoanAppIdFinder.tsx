  ...
  const handleFindLoanDetails = async () => {
    ...
    try {
      const response = await apiFetch(
        `/api/loan-details?searchTerm=${searchTerm}`
      );
      if (!response.ok) {
        ...
        <SignedOut>
          <div className="p-4 text-center">
            ...
            <p className="text-gray-500 mb-4">
              Sign in to access the Universal Loan Finder and join our exclusive
              waitlist for early access to new features.
            </p>
            <SignInButton mode="modal">
        ...
        <SignedIn>
          <div className="p-4">
            <h2 className="text-2xl font-bold text-gray-800">
              Universal Loan Finder
            </h2>
            <p className="text-gray-500">
              Enter a Loan No, App ID, or App. Reg. No. to find loan details.
            </p>
          </div>
          <div className="grid w-full items-center gap-4 py-4 px-4">
            <div className="flex flex-col space-y-1.5">
              <Label htmlFor="searchTerm" className="text-gray-600">
                Search Term
              </Label>
              <Input
                id="searchTerm"
              ...
              <div className="mt-4 space-y-3 w-full bg-gray-50 p-4 rounded-lg border">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-700">
                    <strong>Applicant Name:</strong> {loanDetails.vchr_applname}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() =>
                      copyToClipboard(loanDetails.vchr_applname, "name")
                    }
                  >
                    {copiedField === "name" ? (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      copyToClipboard(
                        String(loanDetails.int_loanappid),
                        "appid"
                      )
                    }
                  >
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-700">
                    <strong>App. Reg. No.:</strong>{" "}
                    {loanDetails.vchr_appreceivregno}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() =>
                      copyToClipboard(loanDetails.vchr_appreceivregno, "regno")
                    }
                  >
                    {copiedField === "regno" ? (
                      <Check className="h-4 w-4 text-green-500" />
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-700">
                    <strong>Loan Number:</strong> {loanDetails.int_loanno}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() =>
                      copyToClipboard(loanDetails.int_loanno, "loanno")
                    }
                  >
                    {copiedField === "loanno" ? (
                      <Check className="h-4 w-4 text-green-500" />
